...
use bevy::{prelude::{ResMut, Res}, time::Time};
use tokio::sync::mpsc;
...
impl Ping {
    pub fn update_time(&mut self, new_time: f64) {
        let tmp_time = (self.time.unwrap_or(PING_DEFAULT_VALUE)*4. + new_time as f32)/5.;        
...
impl PingList {
    ...
    pub(crate) fn receive_pong(&mut self, pong_id: u64, handle: usize, receive_time: f64) {
        if pong_id != self.id {
            return;
        }
    ...
    fn update(&mut self, to_server: ResMut<mpsc::Sender<NetMessage>>, current_time: f64) {
        if let PingState::None = self.state {
            return;
        } else if let PingState::Wait(wait_time) = self.state {
            if wait_time + PING_WAIT_TIME <= current_time {
                let _res = to_server.try_send(NetMessage::Ping(self.id, self.temp));
  ...
  //              log::info!("Network PingList update set Wait");        
    ...
    pub fn start(&mut self) {
        if !self.is_connected() {
            self.state = PingState::Wait(PING_WAIT_TIME*((0.5 + rand::random::<f32>()/f32::MAX) as f64));
        }
    }
